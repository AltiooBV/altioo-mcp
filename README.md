...
| `core_object_search_by_oql` | Search objects with an OQL query; `audit` adds creation and last-change attribution, `actions` adds the per-object write gates and available stimuli; either for a page of 25 or fewer |
| `core_object_search_by_class` | Search objects of a class by attribute criteria; `audit` as above |
| `core_object_get` | Retrieve a single object by class and ID, with its creation and last-change attribution, the write gates answered for *that object*, and the stimuli its current state accepts |
...
| **Enum values** | The `MCP*` values on the `scope` field of `PersonalToken` and `UserToken` — the four grades and one per toolset, ten in all (`_delta="if_exists"`, so no core class is redefined) |
...
| `mcp_allow_automation_administration` | `false` | Whether the instance's outbound machinery may be written here at all: `Trigger`, `Action` (so `ActionEmail`, whose recipients are OQL queries), the links between them, the `RemoteApplicationConnection` a webhook calls through, `AsyncTask` (so `AsyncSendEmail`, the mail queue the cron drains), the `Oauth2Client`/`OAuthClient` tokens the instance authenticates outward with, and iTop's `AuditRule`/`AuditCategory`/`AuditDomain` data-quality checks. Off, every write tool refuses them whatever the caller's iTop rights say. A trigger with an action behind it is a *standing instruction*: it fires on changes made by anyone, from inside iTop's own request handling, long after the session that wrote it is gone — and no tool here can send mail or call a URL directly, so there is no rights answer that makes staging one equivalent to doing it. On, an operator has decided an assistant may wire up notifications and webhooks. Reads are unaffected either way |
